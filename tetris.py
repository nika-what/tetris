...
class TetrisApp(object):
    ...
    def save_score(self):
        if self.current_user:
            ...
            c = conn.cursor()
            c.execute("UPDATE users SET score = ? WHERE username = ?", (self.score, self.current_user))
            conn.commit()
            conn.close()
...
Press space to continue""" % self.score)
    ...
    def show_leaderboard(self):
        ...
        for i, (username, score) in enumerate(leaders, start=1):
            leaderboard_msg += f"{i}. {username}: {score}\n"

        self.center_msg(leaderboard_msg)
    ...
    def center_msg(self, msg):
        for i, line in enumerate(msg.splitlines()):
            ...
            self.screen.blit(msg_image, (
                ...
                self.height // 2 - msgim_center_y + i * 36))
